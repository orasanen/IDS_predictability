...
% 
% These scripts use training with balanced amount of IDS and ADS in the
% training data set. For varying proportion of IDS, use
% predictability_IDS_ADS_main_revision.m
% 
% For the main analyses, see idsads_analyze_results.m in folder named 
% result_analysis_and_plotting/
% 
% Code by Okko Rasanen (okko.rasanen@aalto.fi). Last update: 23.10.2017
...
mocm_order = 5; % order of the mixed-order Markov chain model (default = 3)
...
pythondir = '/Users/orasanen/anaconda/bin/python'; % location of python executable (for LSTM only)
...
F0prob_MOMC = cell(N,1);
F0prob_LSTM = cell(N,1);
...
for iter = 1:n_repeats    
    ...
    tmp2 = find(ads_cumdur > sum(durs(ids_i)),1);  
    ads_to_include = ads_order(1:tmp2-1);        
    valid_traininds = sort([ids_i;ads_to_include],'ascend');
        ...
        for Q = Q_values
            ...
            for k = testinds'                
                if(~isempty(F0prob_MOMC{k}))
                    F0prob_MOMC{k} = F0prob_MOMC{k}+getSeqLikelihood(F0_Q{k},M,lambda);
                    counts(k) = counts(k)+1;
                else
                    F0prob_MOMC{k} = getSeqLikelihood(F0_Q{k},M,lambda);
                    counts(k) = counts(k)+1;
                end
            end
            ...
            if(doLSTM)
                ...
                for k = 1:size(preds,1)
                    ...
                    F0prob_LSTM{signal}(position) = F0prob_LSTM{signal}(position)+prob;
...
for k = 1:N
    if(~isempty(F0prob_MOMC{k}))    
        F0prob_MOMC{k} = F0prob_MOMC{k}(padlength+1:end);
        F0prob_MOMC{k} = F0prob_MOMC{k}./counts(k);
        F0prob_LSTM{k} = F0prob_LSTM{k}(padlength+1:end);
        F0prob_LSTM{k} = F0prob_LSTM{k}./counts(k);        
    end
end

% Save results in a file

result_filename = sprintf('%s/results/results_%s_%s_usesyllables%d_framesize_%d.mat',curdir,datestr(now),dataset,usesyllables,framesize*1000);
